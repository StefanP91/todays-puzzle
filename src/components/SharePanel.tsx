...
const SOCIAL_BUTTONS = [
  ...
  },
  {
    ...
    label: "Instagram",
    className:
      "bg-gradient-to-br from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] hover:opacity-90",
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" aria-hidden>
...
export default function SharePanel({
  ...
  const [instagramToast, setInstagramToast] = useState("");
  const shareUrl = getShareUrl();
  const sharePageUrl = getSharePageUrl(puzzleNumber, guesses, won);
  ...
  async function handleSocial(id: (typeof SOCIAL_BUTTONS)[number]["id"]) {
    switch (id) {
      ...
      case "twitter":
        ...
        break;
      case "telegram":
        openShareWindow(getTelegramShareUrl(shareText, sharePageUrl));
        break;
      case "viber":
  ...
  return (
    <div className={compact ? "space-y-2" : "space-y-3"}>
      ...
      {instagramToast && (
        <p className="text-xs text-correct font-medium leading-snug px-1">{instagramToast}</p>
      )}
